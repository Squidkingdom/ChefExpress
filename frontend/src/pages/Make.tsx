 ...
 *   Date 2 - Programmer 1: Added PDF export functionality, integrated saved & owned recipes, 
 ...
 * Displays the initial hero area with animations and a button to start planning the culinary week.
 */
const HeroSection: React.FC<HeroSectionProps> = ({ onStartPlanning }) => {
  const { scrollY } = useScroll();
  // Create parallax and fade effects based on scroll position
  ...
  return (
    <div className="min-h-screen font-sans overflow-x-hidden relative">
      ...
      {/* Hero text and button with spring and opacity animations */}
      <motion.div
...
// Ingredient interface for recipes
...
// Meal interface representing a scheduled meal in the planner
interface Meal {
  id: number;
  date: string;
...
// SavedMealPlan interface representing a saved plan
...
const fetchSavedRecipes = async () => {
  const token = localStorage.getItem("token"); // token needed for auth
...
// Fetch owned recipes for the user (those created by the user)
...
const MealPlanner: React.FC = () => {
  ...
  // Handlers for adding/removing meals
  ...
  const handleSelectRecipe = (recipe: Recipe) => {
    if (selectedDate && selectedMealType) {
      ...
      // Replace any existing meal on the same date and type, then add the new one
      setMealPlan(prev => ({
        ...prev,
        meals: [...prev.meals.filter(
  ...
  // Generate PDF of the meal plan and recipes
  const generatePDF = () => {
    const doc = new jsPDF();
    ...
    weekDays.forEach((date) => {
      ...
      ["breakfast", "lunch", "dinner"].forEach((mealType) => {
        ...
        if (meal) {
          doc.setFontSize(12);
          doc.text(
            `${mealType.charAt(0).toUpperCase() + mealType.slice(1)}: ${meal.recipe.name}`,
            30,
            yOffset
          );
          yOffset += 7;
  ...
  // RecipeModal component: for selecting a recipe
  const RecipeModal = () => {
  ...
  // SaveModal for saving the meal plan
  ...
  // Planner component displaying the weekly calendar and saved plans
  const Planner: React.FC = () => (
    <div className="pt-24 p-12">
      ...
      >
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div className="flex items-center gap-4">
              <button
                ...
                className="p-2 hover:bg-gray-700/50 rounded-lg text-gray-400 hover:text-teal-400 transition-colors"
              >
        ...
        {/* Calendar: Displaying days and meal slots */}
        <div className="p-6">
  ...
  // Display saved meal plans below planner
  ...
  // MealSlot component for each cell in the calendar
